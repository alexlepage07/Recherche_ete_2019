...
TLS <- function(mere=c("geo", "log"),
    ...
    if (mere =="geo") {
        TLS_mere <- TLS_geo
        TLS_mere.inv <- TLS_geo.inv}
    if (mere =="log") {
        TLS_mere <- TLS_log
        TLS_mere.inv <- TLS_log.inv}
    
    if (enfant == "geo") {
        TLS_enfant <- TLS_geo
        TLS_enfant.inv <- TLS_geo.inv}
    if (enfant == "log") {
        TLS_enfant <- TLS_log
        TLS_enfant.inv <- TLS_log.inv}
    if (enfant == "gamma") {
        TLS_enfant <- TLS_gamma
        TLS_enfant.inv <- TLS_gamma.inv}
...
fct_score <- function(Data, a0, a1, liste_derivees, F_X, ...) {
    ...
    F_X <- function(x) do.call(pexp, list(x, ...))
    Data <- F_X(Data)
    
    return(-sum(log(apply(Data, 1, dCopule, a0, a1, liste_derivees))))
}
...
temps_deriv <- system.time(
    ...
    liste_derivees <- list_derivees("log", "log", 13)
)[[3]]
